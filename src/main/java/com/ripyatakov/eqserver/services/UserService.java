...
public class UserService {
    ...
    public User registerAnonym() {
        int newId = 0;
        try {
            newId = TokenGenerator.getRussianNumber(repository.findFirstByRoleOrderByIdDesc("anon").getName());
        } catch (NullPointerException exc) {
        }
        newId++;
        return saveUser(new User(0, TokenGenerator.getRussiaCode(newId), null, null, null, "anon"));
    }
